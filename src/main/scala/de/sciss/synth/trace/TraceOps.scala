...
object TraceOps {
  ...
  /** Wraps the body of the thunk argument in a `SynthGraph`, adds an output UGen, and plays the graph
    ...
    * @param  toBundleAsync asynchronous OSC bundles to send along with this play command
    * @param  toBundleSync  synchronous OSC bundles to send along with this play command
  ...
  def tracePlay[A](target: Node = Server.default, outBus: Int = 0,
                   ...
                   addAction: AddAction = addToHead, toBundleSync : Vec[osc.Packet] = Vector.empty,
                   toBundleAsync: Vec[osc.Packet] = Vector.empty)(thunk: => A)
                  (implicit result: GraphFunction.Result[A]): TraceSynth = {
    val fun = new TraceGraphFunction(() => thunk)(result)
    fun.play(target = target, outBus = outBus, fadeTime = fadeTime, addAction = addAction, toBundleSync = toBundleSync,
      toBundleAsync = toBundleAsync)
  }
